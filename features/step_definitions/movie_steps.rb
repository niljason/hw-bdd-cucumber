...
Then(/I should see "(.*)" before "(.*)"/) do |_e1, _e2|
  ...
  expect(page_content.index(_e1)).to be < page_content.index(_e2)
...
When(/I (un)?check the following ratings: (.*)/) do |_uncheck, _rating_list|
  ...
  ratings = _rating_list.split(/\s*,\s*/)
  ratings.each do |rating|
    step %{I #{_uncheck}check "ratings_#{rating}"}
  end
#   pending "Fill in this step in movie_steps.rb"
end
